...
export class InterfaceFile {
  ...
  async onSave(_interfaces: Record<string, NetworkInterface>): Promise<void> {
    let _fileContent = `# This file is auto-generated by ElvisPOS Setup Utility. DO NOT EDIT MANUALLY!\n\n`;
    
    _fileContent += `auto lo\n`;
    _fileContent += `iface lo inet loopback\n\n`;

    for (const _interface of Object.values(_interfaces)) {
      let _mode = 'dhcp';

      _interface.ip = _interface._formGroup.get('ip')?.value;
      _interface.netmask = _interface._formGroup.get('netmask')?.value;
      _interface.gateway = _interface._formGroup.get('gateway')?.value;

      if (_interface.ip && _interface.netmask && _interface.gateway) {
        _mode = 'static';
      }
      ...
      if (_mode === 'static') {
        _fileContent += `  address ${_interface.ip}\n`;
